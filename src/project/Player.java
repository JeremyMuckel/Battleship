...
public class Player
{
	...
	{			
		...
		if
		((user.equals("Computer"))||(user.equals("CPU1"))||(user.equals("CPU2"))||(Project.isAutoSet())||(Project.isLocal()))
			for (i=0;i<5;i++)
				boats[i]=new Ship(Project.getShips(i),0,0,0,0);		
	...
	{
		if (Project.getStats().isShowing())
		{	
			Project.getStats().removeAll();
			Project.getStats().setLayout(new GridLayout(6,3));					
			Project.setData(new JLabel(""));
			Project.getStats().add(Project.getData());
			Project.setData(new JLabel("Player 1",SwingConstants.CENTER));
			Project.getStats().add(Project.getData());
			Project.setData(new JLabel("Player 2",SwingConstants.CENTER));
			Project.getStats().add(Project.getData());				
			Project.setData(new JLabel("Names"));
			Project.getStats().add(Project.getData());
			if (Project.getYou() == 0)			
				resetStats(Project.getYou(),Project.getEnemy());			
			else 
				resetStats(Project.getEnemy(),Project.getYou());			
			Project.getStatistics().getContentPane().add(Project.getStats());
			Project.getStatistics().pack();
			Project.getStatistics().repaint();
		}				
	}	
			
	public static void resetStats(int x,int y)
	{
		Project.setData(new JLabel(Project.getPlayers(x).getUser(),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel(Project.getPlayers(y).getUser(),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel("Shots Taken"));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel(Integer.toString(Project.getPlayers(x).getShots()),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel(Integer.toString(Project.getPlayers(y).getShots()),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel("Hits"));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel(Integer.toString(Project.getPlayers(x).getHits()),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel(Integer.toString(Project.getPlayers(y).getHits()),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel("Shot Accuracy"));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel(Project.getPlayers(x).getAcc(),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel(Project.getPlayers(y).getAcc(),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel("Ships Left"));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel(Integer.toString(Project.getPlayers(x).getShipsLeft()),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());
		Project.setData(new JLabel(Integer.toString(Project.getPlayers(y).getShipsLeft()),SwingConstants.CENTER));
		Project.getStats().add(Project.getData());	
	}
	...
	public boolean isSunk(int x, int y)
	{
		...
		{
			Project.getPlayers(Project.getEnemy()).setShipsLeft();
			if
			((Project.getPlayers(Project.getYou()).getUser().equals("Computer"))||(Project.getPlayers(Project.getYou()).getUser().equals("CPU1"))||(Project.getPlayers(Project.getYou()).getUser().equals("CPU2")))
			{	
				for (int k=0;k<10;k++)
					for (int m=0;m<10;m++)
						if
				(this.boats[f].getName().equals(this.getWhatShip(k
							,m)))
						{
							Project.getPlayers(Project.getYou()).setMHS(k,m,2);
							this.setBboard(k,m,Color.black);
						}
				Project.getPlayers(Project.getYou()).setGo(2);				
				Project.getPlayers(Project.getYou()).setChit(false);
	...
	if (((isValid(x+1,y))&&((Project.getPlayers(Project.getYou()).getMHS(x+1,y)==3)||
							(Project.getPlayers(Project.getYou()).getMHS(x+1,y)==1)))||
			((isValid(x-1,y))&&((Project.getPlayers(Project.getYou()).getMHS(x-1,y)==3)||
							(Project.getPlayers(Project.getYou()).getMHS(x-1,y)==1))))		
			u=0;			
		else
			u=5;			
		...
		while (u<5)				
		{
			...
			{						
				...
				{							
					if ((isValid(i,y))&&((Project.getPlayers(Project.getYou()).getMHS(i,y)==3)||
						(Project.getPlayers(Project.getYou()).getMHS(i,y)==1)))
					{
						g+=1;
	...
	public boolean rshipsh(int x,int y)
	{
		...
		if (((isValid(x,y+1))&&((Project.getPlayers(Project.getYou()).getMHS(x,y+1)==3)||
							(Project.getPlayers(Project.getYou()).getMHS(x,y+1)==1)))||
			((isValid(x,y-1))&&((Project.getPlayers(Project.getYou()).getMHS(x,y-1)==3)||
							(Project.getPlayers(Project.getYou()).getMHS(x,y-1)==1))))		
			u=0;			
		else
			u=5;			
		...
		while (u<5)
		{
			...
			{	
				...
				{												
					if ((isValid(x,i))&&((Project.getPlayers(Project.getYou()).getMHS(x,i)==3)||
						(Project.getPlayers(Project.getYou()).getMHS(x,i)==1)))
					{
						g+=1;
						if (g==this.boats[u].getLength())
							break daloop;
	...
	public void scanArea(int x, int y)
	{
		if (this.getGo()==2)
		{
			if (this.isPlausible(x,y+1))
				{
					if (!Project.getPlayers(Project.getEnemy()).rshipsh(x,y+1))
					{	
						this.setMHS(x,y+1,0);
						this.scanArea(x,y);
					}								
				...
				{
					if (!Project.getPlayers(Project.getEnemy()).rshipsv(x+1,y))
					{	
						this.setMHS(x+1,y,0);
						this.scanArea(x,y);
					}							
	...
	{				
		this.setShots();
		if (Project.getPlayers(Project.getEnemy()).getHitOrMiss(x,y))
		{
			this.setHits();
			if (!Project.getPlayers(Project.getEnemy()).isSunk(x,y))
			{
				Project.getPlayers(Project.getEnemy()).setBboard(x,y,Color.orange);
		...
		{	
			Project.getPlayers(Project.getEnemy()).setBboard(x,y,Color.blue);
			if ((this.getUser().equals("Computer"))||(this.getUser().equals("CPU1"))||(this.getUser().equals("CPU2")))
			{	
				this.setMHS(x,y,0);
				this.setChit(false);
	...
	public void humanAttack(ActionEvent v)
	{
		...
		{				
			...
			{				
				for (j=0;j<10;j++)
				{					
					if (source==Project.getPlayers(Project.getEnemy()).getBboard(i,j))
					{								
						if ((Project.getPlayers(Project.getEnemy()).getBboard(i,j).getBackground()==Color.black)||
							(Project.getPlayers(Project.getEnemy()).getBboard(i,j).getBackground()==Color.orange)||
							(Project.getPlayers(Project.getEnemy()).getBboard(i,j).getBackground()==Color.blue))
						{
			...
			this.setMove(false);
			this.getTimer().stop();				
			if (Project.getPlayers(Project.getEnemy()).getShipsLeft()!=0)
			{						
				if (!Project.getPlayers(Project.getEnemy()).getUser().equals("Computer"))
					Project.getPlayers(Project.getEnemy()).setMove(true);					
				Project.getPlayers(Project.getEnemy()).getTimer().start();
				Project.flipYou();
			}
			else
			{
				if (Project.getPlayers(Project.getEnemy()).getUser().equals("Computer"))//change once
					//menu options work
				{
	...
	{	
		...
		{				
			Project.getPlayers(Project.getYou()).compattack();			
		}
	}
	...
	{					
				...
				{							
					...
					{									
						...
						if (this.isSurrounded(r,c))							
							this.setMHS(r,c,0);							
						else if ((!Project.getPlayers(Project.getEnemy()).rshipsh(r,c))
							&&(!Project.getPlayers(Project.getEnemy()).rshipsv(r,c)))
						{							
							this.setMHS(r,c,0);
						}															
						else if ((Project.getPlayers(Project.getEnemy()).getShipsLeft()==1)
							&&(this.getLastShip()==0))
						{								
							for (int i=0;i<5;i++)
								if (Project.getPlayers(Project.getEnemy()).boats[i].getHitsLeft()!=0)
									this.setLastShip(Project.getPlayers(Project.getEnemy()).boats[i].getLength());								
						}							
						rows.clear();
						cols.clear();
					}
		...
		if (Project.getPlayers(Project.getEnemy()).getShipsLeft()>0)
		{
			if ((!Project.getPlayers(Project.getEnemy()).getUser().equals("CPU1"))
				&&(!this.getUser().equals("CPU2")))
				Project.getPlayers(Project.getEnemy()).setMove(true);	
			Project.getPlayers(Project.getEnemy()).getTimer().start();
			Project.flipYou();
		...
		else
		{
			if (this.getUser().equals("Computer"))
			{
				...
				JOptionPane.INFORMATION_MESSAGE);
				if (Project.getPlayers(Project.getEnemy()).getUser().equals("Stupid"))
				...
				for (i=0;i<10;i++)
				{
					for (j=0;j<10;j++)
					{
						if ((!this.getWhatShip(i,j).equals(" "))
							...
							{
								this.setBboard(i,j,Project.getColor());
							}
					}
				}									
			}
