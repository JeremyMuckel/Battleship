...
public class BattleshipClient {   
	...
	{		
		...
		{			
			...
			{
				Project.getPlayers(1).setUser(in.readLine());	
			}				
			System.out.println("I'm still in the loop man.");
		}
		if (!start.equals("getmove"))			
		{
			Project.getPlayers(0).setMove(true);
			JOptionPane.showMessageDialog(null,Project.getPlayers(0).getUser()
	...
	public void results()
	{
		...
		{
			Project.getPlayers(1).setBboard(x,y,Color.blue);
			Project.getPlayers(0).setShots();
		}
		else if (results.startsWith("hit"))
		{
			Project.getPlayers(0).setHits();			
			results=results.substring(results.indexOf(" ")+1);			
			if (results.startsWith("shipsunk"))
			{
				Project.getPlayers(1).setShipsLeft();
				results=results.substring(results.indexOf(" ")+1);
				...
				{					
					for (i=x;i<=endp;i++)
					{
						Project.getPlayers(1).setBboard(i,y,Color.black);
					}
				}
				else
				{
					for (i=y;i<=endp;i++)
					{
						Project.getPlayers(1).setBboard(x,i,Color.black);
					}					
				}
			}			
			else
			{			
				Project.getPlayers(1).setBboard(x,y,Color.orange);
			}
			Project.getPlayers(0).setShots();			
		}
		else if (results.startsWith("theirshot"))
		{
			...
			x=Integer.parseInt(results.substring(0,1));
			y=Integer.parseInt(results.substring(2,3));
			if (!Project.getPlayers(0).getWhatShip(x,y).equals(" "))
			{
				Project.getPlayers(1).setHits();
				if (!Project.getPlayers(0).isSunk(x,y
					,Project.getPlayers(0).getWhatShip(x,y)))
					Project.getPlayers(0).setBboard(x,y,Color.orange);
			}
			else
			  Project.getPlayers(0).setBboard(x,y,Color.blue);
			Project.getPlayers(1).setShots();				
		}
		...
		try
		{
		  ...
		  {
		  		Project.setGameOver(true);
				if (Project.getPlayers(0).getShipsLeft()!=0)
	...
	public void fireShot(int a, int b)
	{
		...
		out.println(x);
		out.println(y);
		System.out.println("shot fired:  "+Project.getCletters(x+1)+","+Project.getCnumbers(y+1));		
	}
	...
	{		
		out.println(Project.getPlayers(Project.getYou()).getUser());			
		for (i=0;i<10;i++)
			for (j=0;j<10;j++)
				out.println(Project.getPlayers(Project.getYou()).getWhatShip(i,j));		
	}
}
